...
export class MimeType<T = Uint8Array> {
    ...
    public toMetadata(payload: Uint8Array | ByteReader | T, hasPayload: boolean = true): Metadata<T> {
        if (payload instanceof Uint8Array) return this.deserializeMetadata(createReader(payload), hasPayload)
    ...
    public toPayload(payload: Uint8Array | ByteReader | T): Payload<T> {
        if (payload instanceof Uint8Array) return this.deserializePayload(createReader(payload))
     ...
     * Unknown values are represented by a transient generic codec. They are
     * deliberately not cached because wire values may come from untrusted peers.
     * Numeric values keep their identifier so re-encoding remains lossless.
     *
