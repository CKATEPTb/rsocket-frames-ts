import {Metadata, MimeType, WellKnownAuthType} from "@/mimetype";
...
export class RSocketAuth<D> extends MimeType<AuthData<D>> {
    public toMetadata(payload: AuthData<D>): Metadata<AuthData<D>> {
        return new class _ extends Metadata<AuthData<D>> {
    ...
    public readMetadata(reader: ByteReader, hasPayload: boolean = true): Metadata<AuthData<D>> {
        ...
        return new Metadata(this.mimeType, this.identifier, {authType: authType, data: data})
    }
}