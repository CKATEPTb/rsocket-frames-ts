...
describe("RSocket MIME extensions", () => {
    ...
    test("distinguishes absent text data from present empty metadata", () => {
    ...
    test("does not mistake application objects for legacy byte readers", () => {
        const source = {
            i8: () => 1,
    ...
    test.each([
        ...
        expect(decoded.metadata).toBe(value);
        expect(decoded.hasMetadata()).toBe(true);
    });
