...
describe("RSocket transport framing", () => {
    ...
    test("releases a large pending TCP allocation after completion and finish", () => {
        ...
        expect((decoder as unknown as {storage: Uint8Array}).storage.byteLength).toBe(0);
        ...
        expect((small as unknown as {storage: Uint8Array}).storage.byteLength).toBeGreaterThan(0);
        small.finish();
        expect((small as unknown as {storage: Uint8Array}).storage.byteLength).toBe(0);
    });
    ...
    test("enforces a defensive frame limit before buffering a TCP body", () => {
        ...
        expect((decoder as unknown as {storage: Uint8Array}).storage.byteLength).toBeLessThanOrEqual(9);
        decoder.reset();
